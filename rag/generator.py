...
## 版本知识检索规则：
你收到的参考资料中，每条内容前都有来源标记，格式为：[版本:XXX | 文档类型:XXX | 优先级:XXX | 来源:XXX]
    ...
    if context:
        ...
        for i, item in enumerate(context, 1):
            ...
            if priority in ("high", "low"):
                source = item.get("title", "") or item.get("id", "")
                prompt_parts.append(f"[版本:{version} | 文档类型:{doc_type} | 优先级:{priority} | 来源:{source}]")
